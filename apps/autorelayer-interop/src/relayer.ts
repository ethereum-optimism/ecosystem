...
export class Relayer {
  ...
  private async relayMessageViaGasTank({
    ...
    let simulatedRelayGasCost: bigint
    let simulatedNestedMessages: readonly Hash[]
    try {
      const simulateResult = (
        await simulateContract(publicClient, contractArgs)
      ).result
      simulatedRelayGasCost = simulateResult[0]
      simulatedNestedMessages = simulateResult[1]
    } catch (error) {
      msgLog.warn({ err: error }, 'relay failed simulation')
      throw error
    }
    const estimatedClaimGasCost = await publicClient.readContract({
      ...
      args: [BigInt(simulatedNestedMessages.length)],
    })
    // Add a 25% buffer to the simulated gas cost to account for base fee increases.
    const totalGasCost =
      ((simulatedRelayGasCost + estimatedClaimGasCost) * 125n) / 100n
