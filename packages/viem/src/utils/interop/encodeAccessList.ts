...
function lookupEntry(id: MessageIdentifier): Hex {
  ...
  // Apply mask to ensure it fits in uint64
  const blockNumberForLookup = id.blockNumber & maxUint64
  const blockNumberBytes = toBytes(toHex(blockNumberForLookup, { size: 8 }))
  out.set(blockNumberBytes, 12)

  // Convert timestamp to uint64 and place in bytes 20-28
  // Apply mask to ensure it fits in uint64
  const timestampForLookup = id.timestamp & maxUint64
  const timestampBytes = toBytes(toHex(timestampForLookup, { size: 8 }))
  out.set(timestampBytes, 20)

  // Convert logIndex to uint32 and place in bytes 28-32
  // Apply mask to ensure it fits in uint32
  const logIndexForLookup = id.logIndex & maxUint32
  const logIndexBytes = toBytes(toHex(logIndexForLookup, { size: 4 }))
